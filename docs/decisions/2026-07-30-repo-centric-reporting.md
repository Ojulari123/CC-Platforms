...
The design questions are resolved — tracked in `docs/questions.md`.
...
5. **Every repo has a lead + a deputy, and BOTH approve** (co-approvers) —
...
6. **Assignment** of a repo's lead, deputy and department is done by a
   **department admin (of that dept) or a platform admin** — either. Lead/deputy
...
- **Migrations:** a new Identity migration dropping `teams`, `memberships.team_id`,
  ...
  **stays** — that's dept-level, unrelated to teams.)

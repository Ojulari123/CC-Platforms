...
- **`POST /auth/signup`** — **open self-signup.** Creates a plain, **non-admin
  account with no department** (empty `memberships` on `/me` until placed — expected,
...
- **`DELETE /platform/users/{id}`** — **permanent**, platform admin only, and only
  ...
  onboarded** — `users.onboarded_at` still null. That column is stamped the first
...
Unauthenticated routes are keyed per client address, authenticated ones per user —
...
- **Refresh-token reuse** (a revoked token presented again) *does* bump it, on top of
  ...
  minted — the access token already in the thief's hands would keep reading data

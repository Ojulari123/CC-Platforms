...
is delegated to identity, so Pulse verifies tokens locally and never calls identity's DB.
...
| GET | `/github/sync-runs` | scoped | Sync history for repos you can see, newest first (paginated; `?repo_id=`), so you can see why your data is stale |
...
invalid, so start again from `/github/connect`), `already_linked` (that GitHub account
...
from `FRONTEND_URL` plus that fixed set, and **no value from the request reaches it**, so
...
signature is checked twice on these routes: once for the key, and once by the auth
...
paged, scoped like the repository list, so you see sync history for repos you can see.
...
way to tell Pulse about it, because a product being pushed at by identity is exactly the
...
`skipped` `SyncRun`, because "nothing happened" and "nothing was meant to happen" have to be
...
`X-Forwarded-For` is **just a request header, and anyone can send one.** Trusting it while

...
Thin routes: verify who's calling, hand off to app/services/github_oauth. The
...
def connect(user: TokenClaims = Depends(current_user)) -> GitHubConnectResponse:
...
def oauth_callback(code: str = Query(...), state: str = Query(...), db: Session = Depends(get_db)) -> HTMLResponse:
...
@router.post("/sync")
def trigger_sync(user: TokenClaims = Depends(current_user), db: Session = Depends(get_db)) -> dict:
    """Run a sync now, instead of waiting for the daily job — handy for demos and
    first setup. Admin-only. Runs inline and returns what each repo pulled."""
    ...
    return {"runs": [{"repo_id": r.repo_id, "status": r.status, "detail": r.detail} for r in runs]}
